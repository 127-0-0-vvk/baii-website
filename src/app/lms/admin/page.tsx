  ...
  return (
    <div className="space-y-5">
      ...
      <div className="bg-white rounded-2xl p-4 space-y-3" style={{ boxShadow: "0 1px 8px rgba(0,0,0,0.05)" }}>
        ...
        <input value={name} onChange={(e) => setName(e.target.value)} placeholder="Cohort name (e.g. 2026 Batch A)" className="w-full rounded-xl px-3.5 py-2.5 text-sm border border-slate-200 bg-slate-50 focus:outline-none focus:border-slate-300" />
        <div>
        ...
        <div><p className="text-xs font-semibold text-slate-500 mb-1">Programme start (Mon of Sem 1, Wk 1)</p><input type="date" value={start} onChange={(e) => setStart(e.target.value)} className="rounded-xl px-3 py-2 text-sm border border-slate-200 bg-slate-50 focus:outline-none" /></div>
        ...
        <>
          ...
          <div className="bg-white rounded-2xl p-4" style={{ boxShadow: "0 1px 8px rgba(0,0,0,0.05)" }}>
            ...
            <div className="grid grid-cols-2 gap-3">{[1, 2, 3, 4].map((s) => (
              <div key={s}><label className="text-xs font-semibold text-slate-500">{semesterTitle(s).split(" — ")[0]}</label><input type="date" defaultValue={active.semStarts[s] ?? ""} onChange={(e) => e.target.value && patch({ sem: s, start_date: e.target.value })} className="w-full mt-1 rounded-xl px-3 py-2 text-sm border border-slate-200 bg-slate-50 focus:outline-none" /></div>
            ))}</div>
          </div>
...
function PodsTab({ cohorts }: { cohorts: Cohort[] }) {
  ...
  return (
    <div className="space-y-5">
      ...
      <select value={cohortId} onChange={(e) => setCohortId(e.target.value)} className="w-full rounded-xl px-3.5 py-2.5 text-sm border border-slate-200 bg-white focus:outline-none">
      ...
      {!cohortId ? <p className="text-sm text-slate-400">Select a cohort above to manage its pods.</p> : (
        <>
          <div className="flex gap-2">
            <input value={newPod} onChange={(e) => setNewPod(e.target.value)} placeholder="New pod name (e.g. Pod 1)" className="flex-1 rounded-xl px-3.5 py-2.5 text-sm border border-slate-200 bg-white focus:outline-none" />
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="bg-white rounded-2xl p-4 space-y-2" style={{ boxShadow: "0 1px 8px rgba(0,0,0,0.05)" }}>
        ...
        <select value={f.student_id} onChange={(e) => setF((x) => ({ ...x, student_id: e.target.value }))} className="w-full rounded-xl px-3 py-2.5 text-sm border border-slate-200 bg-slate-50 focus:outline-none"><option value="">Select student…</option>{students.map((s) => <option key={s.id} value={s.id}>{s.full_name}</option>)}</select>
        <div className="flex gap-2"><input value={f.amount} onChange={(e) => setF((x) => ({ ...x, amount: e.target.value }))} type="number" placeholder="Amount ₹" className="flex-1 rounded-xl px-3 py-2.5 text-sm border border-slate-200 bg-slate-50 focus:outline-none" /><select value={f.status} onChange={(e) => setF((x) => ({ ...x, status: e.target.value }))} className="rounded-xl px-3 py-2.5 text-sm border border-slate-200 bg-slate-50 focus:outline-none">{["paid", "pending", "partial", "waived"].map((s) => <option key={s}>{s}</option>)}</select></div>
        <input value={f.note} onChange={(e) => setF((x) => ({ ...x, note: e.target.value }))} placeholder="Note (e.g. Sem 1 fee)" className="w-full rounded-xl px-3 py-2.5 text-sm border border-slate-200 bg-slate-50 focus:outline-none" />
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4"><div className="absolute inset-0 bg-black/30 backdrop-blur-sm" onClick={onClose} />
      <motion.div initial={{ scale: 0.95, y: 16 }} animate={{ scale: 1, y: 0 }} className="relative bg-white rounded-2xl p-6 w-full max-w-md shadow-2xl z-10">
        ...
        <form onSubmit={submit} className="space-y-3">
          {[["full_name", "Full name", "text"], ["email", "Email", "email"], ["phone", "Phone", "text"], ["password", "Password (min 8)", "text"]].map(([k, l, t]) => <input key={k} required={k !== "phone"} type={t} placeholder={l} value={f[k as keyof typeof f]} onChange={(e) => setF((x) => ({ ...x, [k]: e.target.value }))} className="w-full rounded-xl px-3.5 py-2.5 text-sm border border-slate-200 bg-slate-50 focus:outline-none focus:border-slate-300" />)}
    ...
    <div className="space-y-5 max-w-lg">
      ...
      <div className="rounded-2xl overflow-hidden bg-white" style={{ boxShadow: "0 1px 8px rgba(0,0,0,0.05)" }}>
        ...
        <div className="p-5 space-y-3">
          <div><label className="text-[11px] font-semibold text-slate-500">Full name</label><input value={name} onChange={(e) => setName(e.target.value)} className="w-full mt-1 rounded-xl px-3.5 py-2.5 text-sm border border-slate-200 bg-slate-50 focus:outline-none focus:border-slate-300" /></div>
          ...
          <div><label className="text-[11px] font-semibold text-slate-500">Phone</label><input value={phone} onChange={(e) => setPhone(e.target.value)} placeholder="+91…" className="w-full mt-1 rounded-xl px-3.5 py-2.5 text-sm border border-slate-200 bg-slate-50 focus:outline-none focus:border-slate-300" /></div>
